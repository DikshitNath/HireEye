...
import { Users, Star, X, RefreshCw, Github, Link as LinkIcon, Loader2, Trash2, Mic, User as UserIcon, CheckCircle, Briefcase } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const { userId } = useAuth(); // Get the unique Clerk ID
  ...
  const triggerAutoEvaluation = async (candidate) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/github/evaluate-profile/${candidate._id}`, { method: 'POST' });
      const data = await response.json();
      if (response.ok) {
  ...
  const deleteCandidate = async () => {
    ...
    try {
      await fetch(`http://localhost:5000/api/candidates/${selectedCandidate._id}`, { method: 'DELETE' });
  ...
  const sendInterviewLink = async (candidateId, candidateEmail) => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/candidates/${candidateId}/send-interview`, { method: 'POST' });
      const data = await response.json();
      if (response.ok) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-6 transition-colors duration-300">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              {/* Dynamic Job Buttons */}
              {Array.from(new Set(candidates.map(c => c.jobId?._id).filter(Boolean))).map(id => {
                const job = candidates.find(c => c.jobId?._id === id)?.jobId;

                // ✨ THE FIX: Strictly verify the job AND its title exist before rendering
                if (!job || typeof job.title !== 'string') return null;

                return (
                  <button
                    key={id}
                    onClick={() => setFilterJobId(id)}
                    className={`px-3 py-1 rounded-md text-[10px] font-bold uppercase tracking-tight transition-all ${filterJobId === id
                      ? 'bg-zinc-900 dark:bg-white text-white dark:text-zinc-900'
                      : 'text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800'
                      }`}
                  >
          ...
          {filteredCandidates.length === 0 ? (
            ...
            filteredCandidates.map((candidate) => {
              ...
              return (
                ...
                >
                  {/* ... The rest of your candidate card UI stays exactly the same ... */}
                  <div className="w-10 h-10 rounded bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center font-bold text-sm text-zinc-600 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-700">
                    {candidate.name.charAt(0)}
                  </div>

                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-0.5">
                      <h3 className="text-sm font-bold text-zinc-900 dark:text-white truncate">{candidate.name}</h3>
                      <span className="text-[8px] font-mono font-bold text-zinc-400 uppercase tracking-widest border border-zinc-200 dark:border-zinc-800 px-1 rounded">
